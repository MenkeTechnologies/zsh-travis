#!/usr/bin/env zunit
#{{{                    MARK:Header
##### Purpose: zsh-travis — plugin-contract pins.
...
@test 'entrypoint stem matches plugin directory basename' {
    ...
    # stem of `travis.plugin.zsh` must equal the parent directory's
    # basename so generated source lines stay copy-pasteable.
    local entry='travis.plugin.zsh'
...
@test 'entrypoint parses cleanly under zsh -n' {
    run zsh -n "$pluginDir/travis.plugin.zsh"
    assert $state equals 0
}
